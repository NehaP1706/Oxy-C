        ...
        for (int g=0; g<cmd.ngroups; g++) {
            ...
            if (cmd.groups[g].natoms == 1) {
                ...
                else
                {
                    ...
                    execute_command(tokens, at, count, start, logs, dir_name, cwd, shell_home, &parse_error, jobs, job_count, next_job_id, fg_pid, false);
                }
            }
            else
            {
                ...
                {
                    printf("SEQUENTIAL\n");
                    execute_sequential(&cmd, tokens, g, count, start, logs, cwd, dir_name, shell_home, &parse_error, jobs, job_count, next_job_id, fg_pid, false);
                }
                else
                {
                    printf("PIPELINING\n");
                    execute_pipeline(tokens, &cmd, g, count, start, logs, dir_name, cwd, shell_home, &parse_error, jobs, job_count, next_job_id, fg_pid, false);
                }
            }
