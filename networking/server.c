    ...
    int main(int argc, char **argv) {
        ...
        ssize_t r = recvfrom(sock, buf, sizeof(buf), 0, (struct sockaddr*)&cliaddr, &cli_len);
        if (r <= sizeof(struct sham_header)) {
        ...
        struct sham_header *hdr = (struct sham_header*)buf;
        size_t fname_len = r - sizeof(struct sham_header);
        if (fname_len >= sizeof(outname)) fname_len = sizeof(outname)-1;  // avoid overflow
        uint32_t expected = ntohl(hdr->seq_num) + fname_len;

        memcpy(outname, buf + sizeof(struct sham_header), fname_len);
        outname[fname_len] = '\0';  // null-terminate

        log_event("RCV DATA SEQ=%u LEN=%u", ntohl(((struct sham_header*)buf)->seq_num), strlen(outname));
